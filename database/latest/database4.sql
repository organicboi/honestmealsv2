...
CREATE TABLE public.orders (
  ...
  created_at timestamp with time zone DEFAULT now(),
  updated_at timestamp with time zone DEFAULT now(),
  delivery_pincode text CHECK (delivery_pincode IS NULL OR delivery_pincode ~ '^[1-9][0-9]{5}$'::text),
  estimated_delivery_hours integer DEFAULT 24,
  delivery_zone_type text,
  CONSTRAINT orders_pkey PRIMARY KEY (id),
  CONSTRAINT orders_customer_id_fkey FOREIGN KEY (customer_id) REFERENCES auth.users(id)
);
CREATE TABLE public.pincode_check_logs (
  id uuid NOT NULL DEFAULT uuid_generate_v4(),
  ...
  CONSTRAINT pincode_check_logs_pkey PRIMARY KEY (id),
  CONSTRAINT pincode_check_logs_user_id_fkey FOREIGN KEY (user_id) REFERENCES auth.users(id)
);
...
CREATE TABLE public.serviceable_pincodes (
  id uuid NOT NULL DEFAULT uuid_generate_v4(),
  pincode text NOT NULL UNIQUE CHECK (pincode ~ '^[1-9][0-9]{5}$'::text),
  city text NOT NULL,
  state text NOT NULL,
  area_name text,
  zone_name text,
  ...
  express_delivery_fee numeric DEFAULT 50,
  standard_delivery_hours integer DEFAULT 24,
  express_delivery_hours integer DEFAULT 4,
  max_delivery_hours integer DEFAULT 48,
  zone_type text DEFAULT 'standard'::text CHECK (zone_type = ANY (ARRAY['metro'::text, 'tier1'::text, 'tier2'::text, 'tier3'::text, 'remote'::text])),
  coverage_quality text DEFAULT 'good'::text CHECK (coverage_quality = ANY (ARRAY['excellent'::text, 'good'::text, 'fair'::text, 'limited'::text])),
  min_order_amount numeric DEFAULT 0,
...
CREATE TABLE public.user_custom_exercises (
  ...
  CONSTRAINT user_custom_exercises_user_id_fkey FOREIGN KEY (user_id) REFERENCES auth.users(id)
);
